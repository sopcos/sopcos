...
sequenceDiagram
    ...
    participant PLC
```
Sensor->>Axon: Telemetry {P:105, V:0.09}
Axon->>Axon: Canonicalize + Hash
Axon->>Synapse: Evaluate (Telemetry + Policy)
Synapse->>Synapse: Match Rule: EXPLOSION-RISK-CORRELATED
...
stateDiagram-v2
    [*] --> Clean
    Clean --> Locked: Synapse Verdict (Explosion Risk)
    Locked --> Dirty: Human Override (Class C)
    Dirty --> Audit: Investigation / Dispute
    Audit --> Clean: OP_STATE_RESET (Auditor Only)
```
