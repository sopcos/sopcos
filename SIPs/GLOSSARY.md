...
| **Core Ledger** | The core immutable ledger and "Immortal Notary" of the system. It anchors the ultimate legal truth, DIDs, and cryptographic fingerprints of all artifacts. | SIP-002 |
| **Identity Ledger** | A functional interface of L1 that resolves User DIDs to explicit Attributes (Organization Link, Role) without parsing DID strings. | SIP-021 |
| **SIC (Standard Industrial Commodity)** | A fungible, divisible asset standard (SIP-020) used for tracking quantities like energy credits or raw materials. Distinct from IDAS (Unique Identity). | SIP-020 |
...
| **ABAC** | **Attribute-Based Access Control.** The authorization model where access is granted based on the intersection of User Attributes (Role, Org) and Asset Policy, rather than static lists. | SIP-021 |
| **Access Verdict** | A cryptographically signed proof generated by a Gateway (Synapse) explaining *why* access was granted (e.g., "User Role=Operator matched Policy"). Can be stored off-chain. | SIP-021 |
